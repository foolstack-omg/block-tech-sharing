...
async function main() {
    ...
    let no = process.env.NODE_NUM ? process.env.NODE_NUM : Math.floor(Math.random() * 50)
    log(no)
                        ...
                        for (let i = 0; i < res.data.data['orderPOS'].length; i++) {
                            if (['INIT_PENDING', 'INIT'].includes(res.data.data['orderPOS'][i].status)) {
                                ...
                                switch (res.data.data['orderPOS'][i].token) {
                                    case 'TREAT': {
                                        let TREAT_MAX_BUY_PRICE = res.data.data['orderPOS'][i].price
                                        if (!(TREAT_ORACLE_SELL_PRICE > 0) || !(TREAT_ORACLE_BUY_PRICE > 0)) break
                                        if (TREAT_MAX_BUY_PRICE > TREAT_ORACLE_SELL_PRICE && TREAT_MAX_BUY_PRICE / TREAT_ORACLE_BUY_PRICE * 100 - 100 > 10 && TREAT_MAX_BUY_PRICE / TREAT_ORACLE_SELL_PRICE * 100 - 100 > 5) {
                                    ...
                                    case 'TRICK': {
                                        let TRICK_MAX_BUY_PRICE = res.data.data['orderPOS'][i].price
                                        if (!(TRICK_ORACLE_SELL_PRICE > 0) || !(TRICK_ORACLE_BUY_PRICE > 0)) break
                                        if (TRICK_MAX_BUY_PRICE > TRICK_ORACLE_SELL_PRICE && TRICK_MAX_BUY_PRICE / TRICK_ORACLE_BUY_PRICE * 100 - 100 > 10 && TRICK_MAX_BUY_PRICE / TRICK_ORACLE_SELL_PRICE * 100 - 100 > 5) {
                                    ...
                                    case 'NOSTR': {
                                        let NOSTR_MAX_BUY_PRICE = res.data.data['orderPOS'][i].price
                                        if (!(NOSTR_ORACLE_SELL_PRICE > 0) || !(NOSTR_ORACLE_BUY_PRICE > 0)) break
                                        if (NOSTR_MAX_BUY_PRICE > NOSTR_ORACLE_SELL_PRICE && NOSTR_MAX_BUY_PRICE / NOSTR_ORACLE_BUY_PRICE * 100 - 100 > 10 && NOSTR_MAX_BUY_PRICE / NOSTR_ORACLE_SELL_PRICE * 100 - 100 > 5) {
                                            let order_id = res.data.data['orderPOS'][i].id
                                            if (taked_order_id[order_id]) break
                                    ...
                                    case 'TNA': {
                                        let TNA_MAX_BUY_PRICE = res.data.data['orderPOS'][i].price
                                        if (!(TNA_ORACLE_SELL_PRICE > 0) || !(TNA_ORACLE_BUY_PRICE > 0)) break
                                        if (TNA_MAX_BUY_PRICE > TNA_ORACLE_SELL_PRICE && TNA_MAX_BUY_PRICE / TNA_ORACLE_BUY_PRICE * 100 - 100 > 10 && TNA_MAX_BUY_PRICE / TNA_ORACLE_SELL_PRICE * 100 - 100 > 5) {
